...
from PySide6.QtWidgets import QApplication, QComboBox, QMainWindow, QPushButton, QWizard, QWizardPage, QLineEdit, QHBoxLayout, QLabel, QWidget, QVBoxLayout
from PySide6.QtWidgets import QMainWindow, QMessageBox, QLabel, QPushButton, QLineEdit, QApplication, QFormLayout, QHBoxLayout, QWidget, QTextEdit, QSpinBox, QWizard, QWizardPage
...
# La aplicación consistiría en oulsar a jugar y se elige el juego que se desea jugar,
...
class AnotherWindow(QMainWindow, Ui_MainWindow):
    ...
    # Recuerda que indexes almacena los índices de la selección
        if seleccion.indexes():
            ...
            #indice = self.comboBox_Artist.findText(score)
    ...
    def modificar(self):
        ...
        if self.fila >= 0:
            ...
            #query = QSqlQuery("SELECT nombre FROM estadisticas WHERE Name='"+artista+"'",db=db)
            #while query.next():
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        contenedor= QWidget()
        layout = QVBoxLayout()
        #contenedor.setMinimumSize(100, 20)
